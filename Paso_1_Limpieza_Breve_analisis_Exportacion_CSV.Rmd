...
El objetivo general es **optimizar la eficiencia técnica y financiera de la cartera de seguros de viaje**, mediante la construcción de un modelo de datos robusto que permita **mejorar la rentabilidad operativa** y la sostenibilidad del negocio a largo plazo.
...
- **Identificar patrones de siniestralidad**: Analizar qué variables (destino, producto, corredor) presentan mayor riesgo para la compañía.

- **Optimización de pricing**: Proponer soluciones basados en el análisis del ratio de siniestralidad para maximizar la rentabilidad por segmento.
...
-**Análisis**: Evaluación de la correlación entre la duración del viaje (tramificada) y el índice de siniestralidad.
...
La coherencia del modelo se mantiene mediante la integración de claves: el número de póliza actúa como el eje relacional entre la actividad comercial (primas) y la actividad de riesgo (siniestros), permitiéndonos cruzar datos de ventas con costes de intervención en una sola consulta. 


Se han creado las tablas primeros, se han cargado datos, y una vez cargados se han establecido las relaciones. A continuación se han creado las columnas que necesitamos.

Para la importación de los datos para power bi, se crean las dos vistas de siniestros y primas para abastecer a power bi. Se han hecho left join para poder recibir la información necesaria de siniestros en primas y de primas en siniestros.
...
#sum(primas_limpias_2$Gross_written_premium)
#sum(primas_limpias_2$Number_insured)
#sum(primas_limpias_2$Num_dossier)
#sum(primas_limpias_2$Coste_eur)


write_excel_csv2(dim_garantias, "primas_limpias_2.csv")
...
generar_tabla_rentabilidad_flex <- function(datos, titulo, variable_agrupacion) {
  ...
  tabla_formateada <- final_df %>%
    mutate(across(c(Total_Primas, Total_Primas_Adq, Total_Siniestros, Coste_Medio), ~ paste(format(round(., 0), big.mark = "."), "€"))) %>%
...
- El producto **AV** puro es el más rentable y masivo de todos: tiene un volumen enorme de asegurados (1.14M), un coste medio por siniestro bajo (476 €) y un excelente Ratio con Comisión del 80.93%.

- El producto **AV + anulación clásica** aporta volumen (25.2M €), pero sufre técnicamente más: su ratio de siniestralidad sube al 52.75% y su coste medio por expediente se eleva a 744 €. Añadir la cobertura de cancelación encarece notablemente la severidad de los siniestros.

- El **peligro extremo de la antiselección (Multiviaje)**: El producto Multiviaje presenta un comportamiento técnico alarmante. Con apenas 20.902 € recaudados en primas, ha generado 96.156 € en siniestros,resultando en un Ratio de Siniestralidad del 460.03% y un coste medio por expediente desorbitado de 6.868 €. Esto es un síntoma claro de antiselección: pocos asegurados lo compran, pero los que lo hacen sufren percances extremadamente graves y costosos.
...
En primer lugar, los datos demuestran que el producto estrella (AV pura) es un motor financiero de una estabilidad impecable, manteniendo sus ratios de siniestralidad en la franja del 40% al 42% y su ratio combinado en torno al 80% de manera constante a lo largo de los tres años analizados. 

Por su parte, el producto combinado AV + anulación clásica ha mostrado una notable mejora, logrando reducir su ratio con comisión desde un ajustado 91.77% en 2023 hasta un consolidado 84.61% en 2024, estabilizándose en 2025. Aunque no sería suficiente. EL objetivo sería un 80%.
...
generar_tabla_rentabilidad <- function(datos, titulo) {
  ...
  # 4. Formateo
  tabla_formateada <- final_df %>%
    mutate(across(c(Total_Primas, Total_Primas_Adq, Total_Siniestros, Coste_Medio), 
...
Analizando la serie hist´roica acumulada muestra una dependencia de un único corredor, pero con buenas noticias técnicas:

- **Mundo ideal (MNO) es el motor de la compañía**. Concentra 41.014.927 € de la prima total (casi el 67% de todo el negocio histórico). A pesar de cobrar la comisión más alta de la cartera (40%), su comportamiento del riesgo es muy bueno: una siniestralidad del 40.28% que resulta en un cómodo Ratio con Comisión del 80.28%. Es el cliente más valioso y estable.

- **Viajes sorpresas (MOG)**: Es el segundo corredor por volumen (13.6 millones de €). Sin embargo, su rentabilidad está muy ajustada debido a una alta siniestralidad (62.15%) y un coste medio por expediente elevado (848 €), dejando su ratio combinado al borde del equilibrio con un 94.18%.
...
- **Mundo ideal (MNO)**: Sigue muy estable, facturando 19.0 millones de € con un ratio combinado clavado en el 80.07%.

- **Viajes sorpresas (MOG)**: Ese encuentra en pérdidas técnicas. Registró una prima de 2.2 millones de €, pero su ratio de siniestralidad escaló al 75.66% y el coste medio por siniestro se disparó a 1.027 €. Al sumarle su comisión, alcanzó un Ratio con Comisión del 107.02%.
...
- **Viajes sorpresas (MOG)**: Tuvo su mejor año histórico en volumen con 6.9 millones de €. Aunque su siniestralidad era alta (55.63%), su menor porcentaje de comisión (32.31%) le permitió cerrar el año en una zona segura (pero no suficiente) con un ratio combinado del 87.95%.
...
En este apartado veremos en el **fichero "nombre del fichero"** la visualización de los datos en Power BI.

<br>
<br>

# 8. Análisis detallado en Colab de la base de datos.

En este apartado veremos en el **fichero "nombre del fichero"** el análisis realizado en Colab.
