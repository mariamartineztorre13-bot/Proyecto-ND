...
SELECT 
    COALESCE(CAST(EXTRACT(YEAR FROM policy_start_date) AS VARCHAR), 'TOTAL') AS anio, 
    SUM(gross_written_premium) AS total_primas_neta,
    SUM(Prima_adquirida) AS total_primas_adquiridas, 
    SUM(number_insured) AS total_asegurados,
    SUM(total_dossier) AS total_dossiers_exp,
    SUM(total_coste_siniestros) AS total_dossiers_montante
FROM analiticas.v_fact_primas_con_costes
GROUP BY ROLLUP(EXTRACT(YEAR FROM policy_start_date)) 
ORDER BY EXTRACT(YEAR FROM policy_start_date) ASC NULLS LAST; -- <--


-- 2. Consulta para la vista de siniestros enriquecidos agrupada por año
SELECT 
    COALESCE(CAST(EXTRACT(YEAR FROM policy_start_date) AS VARCHAR), 'TOTAL') AS anio, 
    SUM(coste_eur) AS total_coste_siniestros,     
    SUM(number_insured) AS total_asegurados,
    SUM(num_dossier) AS total_dossiers_cant
