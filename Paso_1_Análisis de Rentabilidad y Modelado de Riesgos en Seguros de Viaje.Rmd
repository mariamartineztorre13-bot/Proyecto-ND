---
title: "Análisis de Rentabilidad y Modelado de Riesgos en Seguros de Viaje"
...
<script>
   $(document).ready(function() {
     $head = $('#header');
     $head.prepend('<img src="Captura_logo.png" style="float: right; width: 15%; margin-top: 60px;"/>')
   });
</script>
```
...
Este análisis permitirá a la empresa ajustar sus políticas de suscripción y mejorar la posición competitiva en el mercado.
...
-**Análisis**: Evaluación de la correlación entre la duración del viaje y el índice de siniestralidad, combinada con la obtención del riesgo predictivo diario mediante regresión logística.
...
En este apartado se detalla el origen y las características de los datos utilizados para el análisis, así como las limitaciones que hemos detectado durante su exploración.
...
Es importante anotar que existe una **relación** entre ambos ficheros a través del número de **póliza**. En el caso de los siniestros, **un mismo expediente puede tener varios mandatos**, lo que provoca que la póliza se repita si el medio de intervención cambia, aunque la causa original del siniestro sea la misma. 
...
- **Excel**: Utilizado en la fase inicial para una exploración visual rápida, lo que permite identificar patrones básicos y detectar las primeras inconsistencias en el formato de los datos.
...
De esta manera **cuando se exporte el "csv"**, **va a identificar aquellas columnas como numéricas** y el **SQL no va a tener problemas** en la lectura ya que están predefinidas las variables.
...
Por último, **se elimina de nuevo duplicados**, ya que anteriomente se han hecho cambios en variables que tras la transformación podría dar el caso de duplicidad:
...
Por último, se procede a la **exportación de los ficheros en formato CSV**, los cuales servirán como **fuente de alimentación** definitiva para nuestro **modelo estrella en SQL**.
...
En este apartado se detallará el **código "nombre fichero" de SQL** para ver las tablas y el modelo estrella creado **que alimentará a Power BI** y **se visualizarán los KPIS**.
...
Señalar que, en la **tabla siniestros no existe una clave primaria única** debido a la naturaleza repetitiva y transaccional de la base de datos. En el diseño de la base de datos de origen ninguna variable individual puede garantizar la unicidad de las filas. La tabla se comporta como un diario de transacciones.
...
Una garantía concreta del catálogo puede sufrir varios siniestros independientes por parte de diferentes clientes a lo largo de los tres años analizados, pero cada apertura de expediente se asigna a una única clave de garantía.
...
**Explicación**: Conecta directamente la dimensión de siniestros con la tabla de hechos de primas utilizando el identificador único de contrato *policy_client*.
...
A continuación, añadimos las columnas calculadas:
...
Creo las columnas necesarias para los análisis y las visualizaciones:
...
Es preciso anotar que este flujo presenta particularidades técnicas:
...
La contraparte necesaria del crecimiento en primas es el control de la siniestralidad. Los datos indican una correlación estrecha entre el volumen de expedientes y el coste total, pero con un matiz importante en la siniestralidad.
...
<br>
Como se puede observar en todos **los gráficos** el mayor número de asegurados **se concentra en el tramo entre 8 y  16 días**.
...
- **Explosión y éxito del B2C**: El canal directo se incrementa hasta los 21.8 millones de €, representando casi el 85% de la producción de este año. Mantiene su rentabilidad, con una siniestralidad en el 40.98%.
...
- El **peligro extremo de la antiselección (Multiviaje)**: El producto Multiviaje presenta un mal comportamiento técnico. Con apenas 20.902 € recaudados en primas (a pesar de ser uno de los productos con prima media más alta), ha generado 96.156 € en siniestros,resultando en un Ratio de Siniestralidad del 460.03% y un coste medio por expediente desorbitado de 6.868 €. Esto es un síntoma claro de antiselección: pocos asegurados lo compran, pero los que lo hacen sufren percances extremadamente graves y costosos.
...
- **Mundo ideal (MNO)**: Sigue muy estable, facturando 19 millones de € con un ratio combinado clavado en el 80.07%. Incrementao de de prima media (**47** € -> **51** € -> **64** €) 
...
Al centrarnos exclusivamente en las métricas de asistencia y costes de siniestralidad, este análisis permite descomponer la frecuencia y la siniestralidad económica de los eventos. 
...
- **Blessé (Accidentados / heridos)**: Es el tercer pilar crítico con 4.786.761 € (17,7%). Al igual que las anulaciones, destaca por una severidad muy alta de 1.070 € por expediente.
...
El último ejercicio se muestra un incremento en la tipología de asistencia médica inmediata:

- **Incremento de Maladie**: Absorbe un récord del 52,94% del presupuesto de siniestros anual (5,3 millones de €).

- Evolución de la gravedad: Los casos de *Blessé* (heridos) escalan al segundo puesto en coste con 1.934.270 € (19,05%), superando los costes totales de *Annulation* (16,6%). No obstante, el coste medio de los accidentes se logra contener por debajo de los mil euros (953 €) por primera vez en tres años.
...
- **Siniestralidad extrema en Blessé** (heridos): En 2023, la gravedad de los accidentes fue sumamente crítica, alcanzando un coste medio de 1.275 € por expediente (el más alto de todo el histórico de la compañía para las categorías principales).

- **La anomalía de Interruption Voyage**: Aunque en los años siguientes se mitiga, en 2023 la interrupción de viaje registró un coste medio alarmante de 1.129 € por caso. (comparando con los 872 € de 2024 o los 889 € de 2025, lo que denota que en 2023 ocurrieron siniestros de repatriación o interrupción inusualmente complejos o mal tarificados).
...
La principal conclusión estratégica se deriva de la siniestralidad económica: mientras que la enfermedad (Maladie) se comporta como un riesgo de alta frecuencia pero coste moderado (~600 €), las Anulaciones y los Accidentes (Blessé) actúan como riesgos de alta siniestralidad, superando sistemáticamente la barrera de los 1.000 € por expediente en el histórico. 
...
**La anomalía catastrófica de 2023 en Blessé (heridos)**: El dato más impactante de toda la serie es el siniestro por accidente en 2023 de 82.356 €. Este único expediente explica por qué la categoría Blessé arrastraba una siniestralidad media tan alta ese año. Es el clásico ejemplo de un siniestro médico grave (probablemente una hospitalización compleja en el extranjero o una repatriación en avión medicalizado).
...
Esto nos permite crear un *storytelling* en base a las tablas y vistas creadas en SQL.
...
Además también se han creado los ratio de margen y el peso de la prima y la siniestralidad en base al total, para ver el peos que tiene cada variable en función del coste, prima, garantía, a través del lenguaje *DAX*.
...
En cuanto a las **recomendaciones**, a partir de la evidencia empírica obtenida se diversas medidas de intervención diseñadas para restaurar la suficiencia de la tarifa y proteger el margen de la compañía. En primer lugar, es importante rediseñar la estructura de tarifas por destino geográfico, aplicando recargos exponenciales al incorporar personas adicionales bajo una misma póliza; esta medida cobra una relevancia significativa en la modalidad de viaje *Mundo* (**OR = 1.072969**), donde el comportamiento expansivo de la probabilidad de ocurrencia exige un ajuste dinámico que compense la exposición en el extranjero. En segundo lugar, se requiere un **replanteamiento profundo de las tarifas aplicadas según el canal de distribución**, siendo el entorno B2B el que mayor urgencia de saneamiento presenta debido a sus reducidos niveles de prima media; en este segmento, la estrategia debe trascender la mera corrección del precio e incorporar un control exhaustivo en la redacción de las condiciones generales y particulares del contrato, limitando de forma estricta las coberturas para mitigar el impacto de la severidad de los siniestros.

En tercer lugar, la entidad debe ir hacia un modelo de **relación comercial más equilibrado** con sus intermediarios mediante la renegociación de los acuerdos de mediación, introduciendo cláusulas de comisiones variables ligadas directamente al ratio de siniestralidad de cada corredor; de este modo, se premiará la fidelidad técnica del mediador manteniendo su nivel de comisión si su cartera es rentable, mientras que se penalizará con una reducción automática de su retribución comercial si los siniestros de su portafolio superan los límites de tolerancia técnica. Finalmente, se recomienda establecer un control estricto de acumulación mediante la imposición de límites al número máximo de asegurados permitidos por póliza individual o, en su defecto, la aplicación sistemática de recargos progresivos por volumen de colectivo, mitigando de raíz el riesgo de concentración de personas en trayectos de larga distancia.
